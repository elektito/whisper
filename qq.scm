  ...
  (let ((rest-n-tail (qq-split-improper-tail form env)))
    (let ((rest (car rest-n-tail))
      ...
      (let ((append-form (list* *qq-append*
                                (map (lambda (x)
                                       (qq-process-list-item x level env))
                                     rest))))
        (if (null? tail)
            append-form
            (append append-form (cons (qq-process-list-tail tail level env) '())))))))

(define (qq-process-list-item form level env)
  (cond ((vector? form) (list *qq-list*
        ...
        ((qq-is-unquote form env)
         (if (= level 1)
             (list *qq-list* (expand-form (cadr form) env))
             (list *qq-list* (qq-process-list form (- level 1) env))))
        ((qq-is-unquote-splicing form env)
         (if (= level 1)
             (expand-form (cadr form) env)
             (list *qq-list* (qq-process-list form (- level 1) env))))
        ((qq-is-quasiquote form env)
         (list *qq-list* (qq-process-list form (+ level 1) env)))
        (else
         (list *qq-list* (qq-process-list form level env)))))

(define (qq-process-list-tail form level env)
  (cond ((vector? form) (list *qq-list->vector*
                              (qq-process-list (vector->list form) level env)))
        ...
        ((qq-is-quasiquote form env)
         (qq-process-list form (+ level 1) env))
        (else
         (qq-process-list form level env))))
