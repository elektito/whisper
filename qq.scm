...
(define (qq-quasiquote form env filename)
  (unless (and (list? form)
               (= (length form) 2)
               (identifier-is-special (car form) 'quasiquote env))
    (compile-error "bad quasiquote form"))
  (let ((result (qq-process (cadr form) 1 env filename)))
    (let ((result (if *qq-simplify* (qq-simplify result) result)))
      (qq-remove-tokens result))))

(define (qq-process form level env filename)
  (cond ((vector? form) (list *qq-list->vector*
                              (qq-process-list (vector->list form) level env filename)))
        ((atom? form) (if (= level 1)
                          (list *qq-quote* form)
                          form))
        ((qq-is-unquote form env)
         (if (= level 1)
             (expand-form (cadr form) env filename)
             (qq-process-list form (- level 1) env filename)))
        ((qq-is-unquote-splicing form env)
         (compile-error "unquote-splicing immediately inside quasiquote"))
        ((qq-is-quasiquote form env)
         (qq-process-list form (+ level 1) env filename))
        (else (qq-process-list form level env filename))))

(define (qq-process-list form level env filename)
  (let ((rest-n-tail (qq-split-improper-tail form env)))
    (let ((rest (car rest-n-tail))
          (tail (cadr rest-n-tail)))
      (let ((append-form (list* *qq-append*
                                (map (lambda (x)
                                       (qq-process-list-item x level env filename))
                                     rest))))
        (if (null? tail)
            append-form
            (append append-form (cons (qq-process-list-tail tail level env filename) '())))))))

(define (qq-process-list-item form level env filename)
  (cond ((vector? form) (list *qq-list*
        ...
        ((qq-is-unquote form env)
         (if (= level 1)
             (list *qq-list* (expand-form (cadr form) env filename))
             (list *qq-list* (qq-process-list form (- level 1) env filename))))
        ((qq-is-unquote-splicing form env)
         (if (= level 1)
             (expand-form (cadr form) env filename)
             (list *qq-list* (qq-process-list form (- level 1) env filename))))
        ((qq-is-quasiquote form env)
         (list *qq-list* (qq-process-list form (+ level 1) env filename)))
        (else
         (list *qq-list* (qq-process-list form level env filename)))))

(define (qq-process-list-tail form level env filename)
  (cond ((vector? form) (list *qq-list->vector*
                              (qq-process-list (vector->list form) level env filename)))
        ...
        ((qq-is-unquote form env)
         (if (= level 1)
             ...
             (qq-process-list form (- level 1) env filename)))
        ((qq-is-unquote-splicing form env)
         (compile-error "unquote-splicing in dotted tail"))
        ((qq-is-quasiquote form env)
         (qq-process-list form (+ level 1) env filename))
        (else
         (qq-process-list form level env filename))))

(define (qq-split-improper-tail ls env)
  ;; () => () -- ()
  ;; (a) => (a) -- ()
  ;; (a b) => (a b) -- ()
  ;; (a . b) => (a) -- b
  ...
  (cond ((null? ls) (list '() '()))
        ((atom? (cdr ls)) (list (list (car ls))
                                (cdr ls)))
        ((qq-is-unquote (cdr ls) env)
         (list (list (car ls))
               (cdr ls)))
        ((qq-is-unquote-splicing (cdr ls) env)
         (list (list (car ls))
               (cdr ls)))
        ((qq-is-quasiquote (cdr ls) env)
         (list (list (car ls))
               (cdr ls)))
        (else (let ((rest-n-tail (qq-split-improper-tail (cdr ls) env)))
                (list (cons (car ls) (car rest-n-tail)) (cadr rest-n-tail))))))

(define (qq-is-unquote form env)
  (and (list? form)
       (= (length form) 2)
       (identifier-is-aux (car form) 'unquote env)))

(define (qq-is-unquote-splicing form env)
  (and (list? form)
       (= (length form) 2)
       (identifier-is-aux (car form) 'unquote-splicing env)))

(define (qq-is-quasiquote form env)
  (and (list? form)
       (= (length form) 2)
       (identifier-is-special (car form) 'quasiquote env)))
    ...
    (if (null? args)
        ...
        (cond ((atom? (car args))
              ...
              ((and (eq? (caar args) *qq-list*)
                    (not (qq-any? (lambda (x) (qq-is-unquote-splicing x #f))
                                  (cdar args))))
               (loop (cdr args)
...
(define (qq-attach-conses items result)
  (cond ((and (qq-all? qq-is-null-or-quoted items)
         ...
         (list *qq-quote* (append (map cadr items)
                                 (if (null? result) '() (cadr result)))))
        ((or (null? result)
...
(define (qq-attach-append op item result)
  (cond ((and (qq-is-null-or-quoted item)
        ...
        ((or (null? result)
             (qq-is-quote-nil result))
         (if (qq-is-unquote-splicing result #f)
             (list op item)
             item))
        ((and (pair? result)
              (eq? (car result) op))
...
(define (qq-remove-tokens form)
  (cond ((eq? form *qq-list*) (identifier 'primcall 'list 'list))
        ((eq? form *qq-append*) (identifier 'primcall 'append 'append))
        ((eq? form *qq-list-star*) (identifier 'primcall 'list* 'list*))
        ((eq? form *qq-quote*) (identifier 'special 'quote 'quote))
        ((eq? form *qq-list->vector*) (identifier 'primcall 'list->vector 'list->vector))
        ((atom? form) form)
        ((and (eq? (car form) *qq-list-star*)
              (pair? (cddr form))
              (null? (cdddr form)))
         (cons (identifier 'primcall 'cons 'cons)
