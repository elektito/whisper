  ...
  (let ((ch (peek-char port)))
    (cond ((eof-object? ch) (read-error "unexpected eof after sharp"))
          ...
          ((char=? #\u ch)
           ...
           (let ((ch (stateful-read-char port)))
             (when (eof-object? ch)
               (read-error "unexpected eof after sharp-u"))
             (unless (char=? #\8 ch)
               (read-char "invalid literal starting with #u"))
            (let ((ls (read-list port)))
              (if (not (list? ls))
                  (read-error "bad bytevector literal: #u8~s" ls)
                  (let loop ((bv (make-bytevector (length ls)))
                             (ls ls)
                             (i 0))
                    (if (null? ls)
                        bv
