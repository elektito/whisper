...
(define (store-has-var store var)
  (not (not (assoc var (store-vars store) free-identifier=?))))
...
(define (starts-with-seq? ls ellipsis)
  (and (pair? ls)
       (pair? (cdr ls))
       (free-identifier=? ellipsis (cadr ls))))
...
(define (find-ellipsis-idx-in-vector vec ellipsis)
  (let ((veclen (vector-length vec)))
    (let loop ((i 0))
      (if (= i veclen)
          #f
          (if (free-identifier=? ellipsis (vector-ref vec i))
              (- i 1)
              (loop (+ i 1)))))))
...
(define (get-all-pattern-vars pattern literals ellipsis)
  (cond ((free-identifier=? ellipsis pattern) '())
        ((member pattern literals free-identifier=?) '())
...
(define (pp-compile-pattern pat literals ellipsis)
  (cond ((identifier? pat) (if (member pat literals free-identifier=?)
...
(define (expand-vector vec store ellipsis)
  (let loop ((result #()) (i 0) (veclen (vector-length vec)))
    (if (= i veclen)
        result
        (if (and (< i (- veclen 1))
                 (free-identifier=? ellipsis (vector-ref vec (+ i 1))))
            (let ((expanded (expand (vector-ref vec i) store ellipsis)))
              (let inner ((multiplied (multiply-seqs expanded)) (next (+ i 2)))
                (if (and (< next (- veclen 1))
                         (free-identifier=? ellipsis (vector-ref vec next)))
...
(define (expand-pair pair store ellipsis)
  (let loop ((result '()) (item pair))
    (cond ((atom? item)
          ...
          ((free-identifier=? ellipsis (car item))
           (if (and (pair? (cdr item))
                    (free-identifier=? ellipsis (cadr item)))
          ...
          ((and (pair? (cdr item))
           ...
           (let ((expanded (expand (car item) store ellipsis)))
             (let inner ((multiplied (multiply-seqs expanded))
                         (rest (cddr item)))
               (if (and (pair? rest)
                        (free-identifier=? ellipsis (car rest)))
...
(define (compile-syntax-rules form)
  ...
  (let* ((ellipsis (if (identifier? (cadr form)) (cadr form) (identifier 'aux '... '...)))
         (literals (if (identifier? (cadr form)) (caddr form) (cadr form)))
         (rules (let loop ((result '())
                           (rest (if (identifier? (cadr form)) (cdddr form) (cddr form))))
                  (if (null? rest)
                      (reverse result)
