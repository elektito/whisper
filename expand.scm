  ...
  (let* ((cu (expand-root-env-compilation-unit env))
    ...
    (cond
     ((and (not past-imports?)
           (pair? form)
           (eq? (car form) 'import))
      (process-import form env filename)
            ...
            (let ((binding (resolve-head head env)))
              (cond ((not binding)
                    ...
                    ((binding-is-special binding 'define-syntax)
                     (process-define-syntax form env filename)
                     '())
...
(define (process-import form env filename)
  (let loop ((import-sets (cdr form)))
    (unless (null? import-sets)
      (let loop-alist ((alist (process-import-set (car import-sets) filename)))
        (unless (null? alist)
          (let* ((entry (car alist))
                 (name (car entry))
                 (kind (cadr entry))
...
(define (process-import-set form filename)
  (unless (list? form)
    (compile-error "invalid import-set: ~a" form))
  (case (car form)
    ((only) (process-import-set-only form filename))
    ((except) (process-import-set-except form filename))
    ((prefix) (process-import-set-prefix form filename))
    ((rename) (process-import-set-rename form filename))
    (else (process-import-set-library form filename))))

(define (process-import-set-library lib-name filename)
...
(define (process-import-set-only/except form clause-name keep? filename)
  (let* ((base-import-set (cadr form))
         (base-bindings (process-import-set base-import-set filename))
         (names (cddr form)))
...
(define (process-import-set-only form filename)
  (process-import-set-only/except form "only" #t filename))

(define (process-import-set-except form filename)
  (process-import-set-only/except form "except" #f filename))

(define (process-import-set-rename form filename)
...
(define (process-import-set-prefix form filename)
  (when (null? (cddr form))
...
(define (resolve-import-binding x lib-name macs filename)
  (let ((name (car x))
        (kind (cadr x)))
    (if (and (pair? (cddr x)) (eq? 'from (caddr x)))
        (let* ((origin-lib (cadddr x))
               ...
               (value (case kind
                        ...
                        ((macro) (caddr (assq origin-name (import-library origin-lib filename))))
...
(define (import-library lib-name filename)
        ...
        (begin
          ...
          (let ((lib (*find-library* lib-name))
            ...
            (let loop ((imports (library-imports lib)))
              (unless (null? imports)
                (process-import (car imports) lib-env filename)
                (loop (cdr imports))))
            (for-each (lambda (name)
                      ...
                      (library-defines lib))
            (let* ((macs (compile-lib-macros lib lib-env filename))
                   (results (map (lambda (x) (resolve-import-binding x lib-name macs filename))
                                 (library-exports lib))))
...
(define (compile-lib-macros lib env filename)
  (let loop ((macs (library-macros lib))
             (results '()))
    (if (null? macs)
        results
        (let ((name (cadar macs))
              (transformer (process-define-syntax (car macs) env filename)))
...
(define (process-define-syntax form env filename)
  (let* ((transformer (compile-transformer (caddr form) env filename))
...
(define (compile-transformer form env filename)
  ...
  (let* ((bindings (cadr form))
    ...
    (for-each (lambda (b id)
                (binding-meaning-set! (identifier-binding id)
                                      (compile-transformer (cadr b) def-env filename)))
              bindings ids)
