  ...
  (let ((port (open-output-file filename))
    ...
    (if (program-library-mode program)
        (begin
          ...
          (display "    value home = find_library_env(provided_libs);\n" port)
          (display "    if (!home) {\n" port)
          (display "        home = provided_libs[0] ? make_environment() : env;\n" port)
          (display "        register_library_env(provided_libs, home);\n" port)
          (display "        global_env = home;\n" port)
          (format port "        result = ~a(NULL, ACCEPTS_MVALUES, 0);\n" (func-name (program-init-func program)))
          (display "    }\n" port)
          (display "    if (env != home) {\n" port)
          (display "        for (value **g = provided_globals; *g; ++g) {\n" port)
          (display "            env_delegate(env, **g, home);\n" port)
          (display "        }\n" port)
          (display "    }\n" port)
