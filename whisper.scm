...
(define (gen-register-globals program output)
  (let ((n-symbols (length (program-symbols program))))
    ...
    (format output "    symbols = calloc(sizeof(struct symbol), ~a);\n" n-symbols)
    (let loop ((i 0) (symbols (program-symbols program)))
      (if (not (null? symbols))
          (let* ((sym (car symbols))
                 (info (hash-table-ref/default *primcalls-table* sym #f))
                 (value-expr
                   (if info
                       (let* ((c-name (cadr info))
                              (min-args (caddr info))
                              (max-args (cadddr info))
                              (c-max-args (if (= max-args -1) "MAX_ARGS" max-args)))
                         (format "make_closure(primcall_~a, ~a, ~a, 0)" c-name min-args c-max-args))
                       "VOID")))
...
(define (compile-set! func indent form)
  ...
  (let ((value-varnum (compile-form func indent (caddr form)))
    ...
    (case (meaning-kind meaning)
      ...
      ((free)
       (let* ((freevar-idx (or (func-find-freevar func (cadr form))
                               (func-add-freevar func (cadr form)))))
         (gen-code func indent "primcall_set_box_b(NULL, NO_CALL_FLAGS, 2, envget(env, ~a), x~a);\n" freevar-idx value-varnum)))
      (else
