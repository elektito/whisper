...
(define (gen-provided-libs program output)
  (format output "static const char *provided_libs[] = {\n")
  (for-each
   (lambda (lib)
     (format output "    \"~a\",\n" (library-name lib)))
  ...
  (let ((port (open-output-file filename))
    ...
    (if (program-library-mode program)
        (begin
          ...
          (display "    value result = VOID;\n" port)
          (display "    register_globals();\n" port)
          (display "    if (provided_libs[0]) {\n" port)
          (display "        global_env = get_global_env();\n" port)
          (display "        if (!is_library_registered(provided_libs)) {\n" port)
          (display "            register_library(provided_libs);\n" port)
          (format port "            result = ~a(NULL, ACCEPTS_MVALUES, 0);\n" (func-name (program-init-func program)))
          (display "        }\n" port)
          (display "        if (env != global_env) {\n" port)
          (display "            for (value **g = provided_globals; *g; ++g) {\n" port)
          (display "                env_delegate(env, **g, global_env);\n" port)
          (display "            }\n" port)
          (display "        }\n" port)
          (display "    } else {\n" port)
...
(define *primcalls* '((%display "percent_display" 2 2)
                      (%exit "percent_exit" 2 2)
                      (%flush-output-port "percent_flush_output_port" 1 1)
                      (%gc-stats "percent_gc_stats" 0 0)
                      (%make-hash-table "percent_make_hash_table" 2 2)
                      (%newline "percent_newline" 1 1)
                      (%peek-char "percent_peek_char" 1 1)
                      (%read-char "percent_read_char" 1 1)
                      (%read-line "percent_read_line" 1 1)
                      (%u8-ready? "percent_u8_ready_q" 1 1)
                      (%unread-char "percent_unread_char" 2 2)
                      (%write "percent_write" 2 2)
                      (%write-char "percent_write_char" 2 2)
                      (abort "abort" 1 1)
                      (append "append" 0 -1)
                      (apply "apply" 1 -1)
                      ...
                      (box "box" 1 1)
                      (box? "box_q" 1 1)
                      (bytevector? "bytevector_q" 1 1)
                      (bytevector-length "bytevector_length" 1 1)
                      (bytevector-u8-ref "bytevector_u8_ref" 2 2)
                      (bytevector-u8-set! "bytevector_u8_set_b" 3 3)
                      (%call/cc "callcc" 1 1)
                      ...
                      (floor "floor" 1 1)
                      (gc "gc" 0 0)
                      (gc-manual-mode! "gc_manual_mode_b" 1 1)
                      (gensym "gensym" 0 1)
                      ...
                      (list "list" 0 -1)
                      (list* "list_star" 1 -1)
                      (list->vector "list_to_vector" 1 1)
                      (make-bytevector "make_bytevector" 1 2)
                      ...
                      (set-system-exception-handler "set_system_exception_handler" 1 1)
                      (stderr "stderr" 0 0)
                      (stdin "stdin" 0 0)
                      (stdout "stdout" 0 0)
...
(define *inline-primcalls* '((boolean? 1 value () ("BOOL(IS_BOOL(" 0 "))"))
                             ...
                             (car 1 value
                              ...
                              ("GET_PAIR(" 0 ")->car"))
                             (caar 1 value
                              ((("!IS_PAIR(" 0 ")") . "caar argument is not a pair")
                               (("!IS_PAIR(GET_PAIR(" 0 ")->car)") . "caar argument's car is not a pair"))
                              ("GET_PAIR(GET_PAIR(" 0 ")->car)->car"))
                             (cadr 1 value
                              ((("!IS_PAIR(" 0 ")") . "cadr argument is not a pair")
                               (("!IS_PAIR(GET_PAIR(" 0 ")->cdr)") . "cadr argument's cdr is not a pair"))
                              ("GET_PAIR(GET_PAIR(" 0 ")->cdr)->car"))
                             (cdar 1 value
                              ((("!IS_PAIR(" 0 ")") . "cdar argument is not a pair")
                               (("!IS_PAIR(GET_PAIR(" 0 ")->car)") . "cdar argument's car is not a pair"))
                              ("GET_PAIR(GET_PAIR(" 0 ")->car)->cdr"))
                             (cddr 1 value
                              ((("!IS_PAIR(" 0 ")") . "cddr argument is not a pair")
                               (("!IS_PAIR(GET_PAIR(" 0 ")->cdr)") . "cddr argument's cdr is not a pair"))
                              ("GET_PAIR(GET_PAIR(" 0 ")->cdr)->cdr"))
                             (cdr 1 value
                             ...
                             (fixnum? 1 value () ("BOOL(IS_FIXNUM(" 0 "))"))
                             (flonum? 1 value () ("BOOL(IS_FLONUM(" 0 "))"))
                             ...
                             (number? 1 value () ("BOOL(IS_FIXNUM(" 0 ") || IS_FLONUM(" 0 "))")) ;; TODO fix this when we have other number types
...
;; arguments are never in tail position, and their values are used (by
;; the call being made), never discarded.
(define (compile-list-of-forms func indent forms)
  (let loop ((varnums '()) (forms forms))
    (if (null? forms)
        (reverse varnums)
        (loop (cons (compile-form func indent (car forms) #f #f) varnums)
              (cdr forms)))))
...
;; the ACCEPTS_MVALUES bit tells the callee whether returning something
;; other than exactly one value is acceptable. a genuine tail call
;; forwards whatever the enclosing function was itself called with (its
;; caller decides); a discarded, non-tail position can unconditionally
;; accept any number of values since nothing ever looks at the result;
;; anything else (the value is used, but not as this function's own
;; return value) must stay single-valued.
(define (call-flags-string tail? discard?)
  (cond (tail? "IN_TAIL_POSITION | (flags & ACCEPTS_MVALUES)")
        (discard? "ACCEPTS_MVALUES")
        (else "NO_CALL_FLAGS")))

(define (compile-primcall func indent form primcall-info tail? discard?)
...
(define (compile-quoted-item func indent form)
  (cond ((boolean? form) (compile-form func indent form #f #f))
        ((number? form) (compile-form func indent form #f #f))
      ...
      (let ((new-func (add-function (func-program func) func))
        ...
        (let loop ((body (cddr form)) (varnum -1))
          (if (null? body)
              ...
              (let ((form (car body))
                    (last? (null? (cdr body))))
                (loop (cdr body) (compile-form new-func 1 form last? (not last?))))))
...
(define (compile-let func indent form tail? discard?)
...
(define (compile-if func indent form tail? discard?)
  (let ((cond-varnum (compile-form func indent (cadr form) #f #f)))
    (let ((ret-varnum (func-next-varnum func)))
      (gen-code func indent "value x~a = VOID;\n" ret-varnum)
      (gen-code func indent "if (x~a != FALSE) {\n" cond-varnum)
      (let ((then-varnum (compile-form func (+ indent 1) (caddr form) tail? discard?)))
        (gen-code func (+ indent 1) "x~a = x~a;\n" ret-varnum then-varnum)
        (gen-code func indent "} else {\n")
        (let ((else-varnum (compile-form func (+ indent 1) (cadddr form) tail? discard?)))
          (gen-code func (+ indent 1) "x~a = x~a;\n" ret-varnum else-varnum)
          (gen-code func indent "}\n")))
      ret-varnum)))

(define (compile-begin func indent form tail? discard?)
  (when (= 1 (length form))
    (compile-error "empty begin expression is not allowed"))
  (let ((ret-varnum (func-next-varnum func)))
    (let loop ((exprs (cdr form)))
      (let* ((last? (null? (cdr exprs)))
             (expr-varnum (compile-form func indent (car exprs)
                                        (and tail? last?)
                                        (or (not last?) discard?))))
        (if last?
...
(define (compile-set! func indent form)
  (let ((value-varnum (compile-form func indent (caddr form) #f #f))
        (b (identifier-binding (cadr form))))
    (case (binding-kind b)
      ((global)
       (intern (func-program func) (binding-meaning b))
...
(define (compile-special func indent form kind tail? discard?)
  (case kind
    ((begin) (compile-begin func indent form tail? discard?))
    ((define) (compile-define func indent form))
    ((if) (compile-if func indent form tail? discard?))
    ((let) (compile-let func indent form tail? discard?))
    ((letrec) (compile-letrec func indent form tail? discard?))
    ((letrec*) (compile-letrec* func indent form tail? discard?))
...
(define *root-identifiers* (list (identifier 'special 'begin 'begin)
                                 ...
                                 (identifier 'primcall 'box 'box)
                                 (identifier 'primcall 'bytevector-length 'bytevector-length)
                                 (identifier 'primcall 'bytevector-u8-ref 'bytevector-u8-ref)
                                 (identifier 'primcall 'bytevector-u8-set! 'bytevector-u8-set!)
                                 (identifier 'primcall 'bytevector? 'bytevector?)
                                 (identifier 'primcall '%call/cc '%call/cc)
                                 ...
                                 (identifier 'primcall 'cdar 'cdar)
                                 (identifier 'primcall 'cddr 'cddr)
                                 (identifier 'primcall 'cdr 'cdr)
                                 (identifier 'primcall 'ceiling 'ceiling)
                                 ...
                                 (identifier 'primcall 'exact 'exact)
                                 (identifier 'primcall 'fixnum? 'fixnum?)
                                 (identifier 'primcall 'flonum? 'flonum?)
                                 (identifier 'primcall 'floor 'floor)
                                 (identifier 'primcall 'gc 'gc)
                                 (identifier 'primcall 'gc-manual-mode! 'gc-manual-mode!)
                                 ...
                                 (identifier 'primcall 'inexact 'inexact)
                                 (identifier 'primcall 'input-port? 'input-port?)
                                 (identifier 'primcall 'integer->char 'integer->char)
                                 (identifier 'primcall 'list 'list)
                                 (identifier 'primcall 'list-directory 'list-directory)
                                 (identifier 'primcall 'list* 'list*)
                                 (identifier 'primcall 'list->vector 'list->vector)
                                 (identifier 'primcall 'make-bytevector 'make-bytevector)
                                 ...
                                 (identifier 'primcall 'not 'not)
                                 (identifier 'primcall 'null? 'null?)
                                 ...
                                 (identifier 'primcall 'set-system-exception-handler 'set-system-exception-handler)
                                 (identifier 'primcall 'stderr 'stderr)
                                 (identifier 'primcall 'stdin 'stdin)
                                 (identifier 'primcall 'stdout 'stdout)
                                 ...
                                 (identifier 'primcall 'truncate 'truncate)
          ...
          (let ((decl (caar decls))
            ...
            (case (car decl)
              ...
              ((c-include)
               ...
               (program-add-c-include program (resolve-filename (cadr decl) filename))
    ...
    (case library-mode
      ...
      ((library)
       (let* ((obj (string-append (temp-filename) ".o"))
              (so-cmd (format "~a -I~a ~a -DSO_MODE -fPIC -shared -o ~a.so ~a -L~a -L~a ~a" cc core-path cflags out-file c-file here out-dir so-extra-flags))
