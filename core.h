...
static value make_closure_minmax(funcptr func, int min_args, int max_args, int nfreevars, ...) {
    va_list args;
...
/* Shim for code generated by whisper-v5. Remove when bumping to v7. */
static value make_closure(funcptr func, int nargs, int nfreevars, ...) {
    va_list args;
    int min_args = (nargs == -1) ? 0 : nargs;
    int max_args = (nargs == -1) ? MAX_ARGS : nargs;
    struct closure *closure = alloc_closure(nfreevars);
    closure->func = func;
    closure->min_args = min_args;
    closure->max_args = max_args;
